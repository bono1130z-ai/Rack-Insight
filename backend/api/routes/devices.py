...
from schemas.device import DeviceCreate, DeviceDetailResponse, DeviceResponse, DeviceUpdate
...
@router.post("/{device_id}/refresh", response_model=DeviceInventoryResponse)
async def refresh(
    device_id: uuid.UUID, db: AsyncSession = Depends(get_db)
) -> DeviceInventoryResponse:
    try:
